...
import com.pinduoduo.service.sys.ForestryService;
...
public class ForestryController extends ForestryBaseController<Forestry> {
		...
		if (!file.isEmpty()) {
			if (file.getSize() > 2097152) {
				...
				try {
					...
					sheet = wb.getSheetAt(0);// excel涓嚦灏戜細瀛樺湪涓�涓猻heet椤�
					int rowNum = sheet.getPhysicalNumberOfRows();// 鐗╃悊鏈夋晥琛屾暟
					Object[] rowValues = null;// excel涓竴琛屾爲鏈ㄤ俊鎭�
					List<Object[]> models = new ArrayList<Object[]>();// excel涓叏閮ㄦ爲鏈ㄤ俊鎭�
					if (rowNum > 1) {
						...
						columns: for (int i = 1; i < rowNum; i++) {
							...
							if (currentRow != null) {
								...
								// int cellNum = currentRow.getLastCellNum();// 鎬诲崟鍏冩牸鏁扮洰
								for (short j = 0; j < 5; j++) {
									try {
											...
											switch (currentCell.getCellType()) {
												...
												case Cell.CELL_TYPE_NUMERIC:
													if (HSSFDateUtil.isCellDateFormatted(currentCell)) {
														...
														nf.setGroupingUsed(false);//true鏃剁殑鏍煎紡锛�1,234,567,890
										...
										rowValues = null;
										stringBuilder.append("绗�" + i + "琛�," + headRow.getCell(j).getRichStringCellValue() + "鍒楄緭鍏ヤ簡闈炴硶鍊硷紝鏈鍏ユ垚鍔燂紒");
										continue columns;
									} catch (NullPointerException e) {
										rowValues = null;
										stringBuilder.append("绗�" + i + "琛�," + headRow.getCell(j).getRichStringCellValue() + "鍒楄緭鍏ヤ簡绌哄�硷紝鏈鍏ユ垚鍔�!");
										continue columns;
									} catch (Exception e) {
										rowValues = null;
					...
					} else if (rowNum <= 1 && rowNum > 0) {// 琛ㄧず妯＄増涓彧瀛樺湪澶撮儴淇℃伅
						json.put("success", false);
						json.put("msg", "Excel琛ㄦ牸涓病鏈夐渶瑕佸鍏� 鐨勫唴瀹癸紒");
						writeJSON(response, json.toString());
						return;
					} else if (rowNum <= 0) {// 琛ㄧず杩欐槸涓�涓┖sheet椤�
						json.put("success", false);
						json.put("msg", "鎵�瀵煎叆鏂囦欢鏍煎紡涓嶆纭紝璇蜂笅杞芥ā鏉匡紒");
						writeJSON(response, json.toString());
						return;
					}
					...
					for (int i = 0; i < list.size(); i++) {
						if (StringUtils.isBlank(list.get(i).getEpcId()) || StringUtils.isBlank(list.get(i).getName())) {
							stringBuilder.append("绗�" + (i + 1) + "琛岃褰曠殑蹇呭～椤规湁绌哄�硷紝瀵煎叆澶辫触銆�");
							continue;
						}
						Forestry checkForestryEpcId = forestryService.getByProerties("epcId", list.get(i).getEpcId());
						if (checkForestryEpcId != null) {
							stringBuilder.append("绗�" + (i + 1) + "琛岃褰曠殑epc缂栫爜宸插瓨鍦紝瀵煎叆澶辫触銆�");
							continue;
						}
						if (list.get(i).getForestryType() == null) {
							stringBuilder.append("绗�" + (i + 1) + "琛岃褰曠殑绉嶇被涓虹┖鎴栦笉瀛樺湪锛屽鍏ュけ璐ャ��");
							continue;
						}
					...
					json.put("msg", count + "鏉¤褰曞鍏ュ畬鎴愩��" + stringBuilder.toString());
				} catch (Exception e) {
					e.printStackTrace();
					json.put("success", false);
	...
	public void exportForestry(HttpServletRequest request, HttpServletResponse response, @RequestParam("ids") Long[] ids) throws Exception {
		...
		//鍒涘缓sheet椤�
		HSSFSheet sheet = workBook.createSheet("鏍戞湪淇℃伅");
		//璁剧疆绗竴琛屼负Header
		...
		cell0.setCellValue("epc缂栫爜");
		cell1.setCellValue("鍚嶇О");
		cell2.setCellValue("绉嶆鏃堕棿");
		cell3.setCellValue("鍏ュ洯鏃堕棿");
		cell4.setCellValue("鎵�灞炵绫诲悕绉�");
